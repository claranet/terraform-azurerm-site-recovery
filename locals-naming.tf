locals {
  ...
  primary_srpc_name   = coalesce(var.primary_site_recovery_protection_container, lower(data.azurecaf_name.primary_srpc))
  secondary_srpc_name = coalesce(var.secondary_site_recovery_protection_container, lower(data.azurecaf_name.secondary_srpc))
}
